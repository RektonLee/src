...
def evaluate(dataset_path, model_path, save_dir="outputs"):
    ...
    #     print(f"{k}: {v:.4f}")
    residuals=residual_analysis(y_true.numpy(), y_pred.numpy(), save_dir)
    ...
    # 绘制残差热图
    plot_residual_heatmap(residuals, os.path.join(save_dir, "residual_heatmap.png"))
    print(f"✅ Residual heatmap saved to {save_dir}/residual_heatmap.png")

def plot_residual_heatmap(residuals, save_path):
...
def gradient_saliency_analysis(model, loader, device):
    model.eval()
    all_gradients = []
