...
SAVE_PATH = 'data/processed/dataset_NAN_nopqr_v2.pt' # 修改了保存路径
...
def cosine_cutoff(edge_dist, cutoff):
    """
    ...
    return torch.where(
        ...
        torch.tensor(0.0, device=edge_dist.device)
    )
...
def build_graph(atoms):
    ...
    directions = (pos[col] - pos[row]) / (distances.unsqueeze(-1) + 1e-6)  # Avoid division by zero
